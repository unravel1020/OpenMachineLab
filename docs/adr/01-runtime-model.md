# ADRs — Phase 1: the runtime model

The foundation decisions: what the model is, how it is laid out, the toolchain,
the lifecycle state, and the run loop. (See the [ADR index](../ADR.md).)
...
stop *signal* crosses a thread boundary.
